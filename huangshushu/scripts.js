...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 送單模組:Token 兩段式 + 自動重試
  // ===========================================================
  const GAS_URL = 'https://script.google.com/macros/s/AKfycbzgAiIPWFfKN4SjNBkAmxFewYDy9Jj2PxN9QnybsJFSF_LsiohB16JtGScmcvJvPuwmqA/exec';
  ...
  async function sendCartToLine(data) {
    try {
      // ★ 直接使用 btnCheckout 時取得的 token,不在這裡重取
      ...
      // ★ 重點:不再自動重取 Token 重送
      //   Token 過期/已用 → 直接讓顧客看到失敗,引導重新下單
      //   這才能讓 TOKEN_TTL 真正起作用
      if (result.ok) {
